...
$query2 = "SELECT * FROM doacao WHERE idProduto='".$l['idProduto']."' AND codDoacao='$codDoacao'";
//echo $query2;
...
if( $l2 != 0 ){
	...
	$query3 = "UPDATE doacao SET qtd='$qtd', total='$total' WHERE idProduto='".$l['idProduto']."' AND codDoacao='$codDoacao'";
	$res3 = mysql_query($query3);
}else{
	# se não adiciona o novo produto
	if(	$l['idProduto'] != "") {
		//echo "upa";
		$query4 = "INSERT INTO doacao VALUES (null,null,'$codDoacao','".$l['idProduto']."','1','".$l['precoVenda']."',null,null)";
...
$query4 = "SELECT p.*, n.natureza, e.quantidade, d.* FROM doacao d
			INNER JOIN produto p ON p.idProduto = d.idProduto			 
				INNER JOIN natureza n ON n.idNatureza = p.idNatureza
					INNER JOIN entrada_produto e ON e.idProduto = p.idProduto
						WHERE d.codDoacao='$codDoacao'";
$res4 = mysql_query($query4);
...
while( $l4 = mysql_fetch_array($res4) ) {
	...
	$carrinho .= "<td><a href='../function/cancelarItemDoacao.php?item=".$l4['idProduto']."&doacao=".$codDoacao."' border=0><img src='../imagens/layout/ico_del.png' /></a></td>";
	$carrinho .= "</tr>";
	$carrinho .= "<tr align='right'><td>&nbsp;</td></tr>";
}
$carrinho .= "</tbody><tr><td colspan=5>&nbsp;</td></tr>";
$carrinho .= "<tr>";
$total = Doacao::totalDoacao($codDoacao);
